...
from yc import Yichuan
from matplotlib.backends.backend_tkagg import NavigationToolbar2Tk
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
from matplotlib.figure import Figure
import networkx as nx
class Inherit(tk.Frame):
    ...
    def run_(self):
        ...
        self.zhubei = Yichuan(self.wulitu, self.luojitu)
        self.Individual= self.zhubei.show()
        # 指定存储路径
        file_path = os.getcwd() + r"\GUI\generatedata\inherit_I.pkl"
        # 确保文件夹存在
        os.makedirs(os.path.dirname(file_path), exist_ok=True)

        # 将实例序列化并保存到文件
        with open(file_path, 'wb') as file:
            pickle.dump(self.Individual, file)

        self.data = self.Individual.peizhipaths
        df = pd.DataFrame(self.data)
        p = df.T
        p.to_excel(self.path_ + '/遗传主备路径选择.xlsx')

        for item in self.data:
            s, e = item
            tuple1 = tuple([s, e] + self.data[item])
        ...
        # 绘制Networkx图
        pos = nx.spring_layout(self.Individual)
        nx.draw(self.Individual, pos, ax=ax, with_labels=True)
